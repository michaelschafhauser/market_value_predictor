...
def transfer_cleaning(transfer_df):
    ...
    df.sort_values(
        ["year", "player_name", "club_name", "fee_cleaned"],
...
def merge_by_name_only(transfer_df, players_df):
    ...
    merged_df = merged_df.drop_duplicates(subset=["player_name", "year"], keep=False)
...
def clean_master_df(df):
    ...
    df["national_team"] = df.nation_jersey_number.map(lambda x: 1 if x > 0 else 0)
    df["16_y_after_dob"] = df.dob + pd.to_timedelta(16 * 365, unit="d")

    new_joined_list = []
    for i, v in enumerate(list(df.joined)):
        if pd.isnull(v):
            new_joined_list.append(
                pd.to_datetime(df[["16_y_after_dob"]].iloc[i]["16_y_after_dob"])
            )
        else:
    ...
    df["seasons_with_club"] = df["fifa year"] - pd.DatetimeIndex(df.joined).year

    df["on_loan"] = df.loaned_from.map(lambda x: 0 if pd.isnull(x) else 1)

    df["remaining_seasons_on_contract"] = df.contract_valid_until - df["fifa year"]
...
if __name__ == "__main__":
    ...
    path_to_player = input("Provide file path to players_combined file:    ")
    players_df = get_data(path_to_player)
    players_df = players_cleaning(players_df)

    path_to_save = input(
        "Provide file path to save final dataframe as csv (don't forget filename including .csv extenstion):   "
    )
    ...
    if with_webscraping == "Y":
        merged_2_df = merge_by_sofifa_id(
            add_sofifa_id_to_non_matched_transfers(transfer_df, players_df), players_df
        )
